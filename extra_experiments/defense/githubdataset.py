...
class GitHubDataset(Dataset):
    ...
    @staticmethod
    def get_samples(root, num=None, shuffle=False, extension='py', exclude=None, return_num_all=False, delete_comment=False):
            ...
            for f in samples:
                try:
                    with open(f, 'r', encoding='utf-8') as inF:
                        ...
                        if delete_comment:  # if comment is deleted during training
                            comment_pattern = re.compile(r'#.*?$|/\*.*?\*/|\'\'\'(.*?)\'\'\'|\"\"\"(.*?)\"\"\'', re.MULTILINE | re.DOTALL)
                            txt = re.sub(comment_pattern, '', txt)
