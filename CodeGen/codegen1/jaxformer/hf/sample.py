...
from codegen1.jaxformer.hf.codegen.modeling_codegen import CodeGenForCausalLM, CodeGenConfig
...
def create_model(ckpt, fp16=True, gradient_checkpointing=False):
    if gradient_checkpointing:
        config = CodeGenConfig.from_pretrained(ckpt)
        config.gradient_checkpointing = True
        config.use_cache = False
        if fp16:
            model = CodeGenForCausalLM.from_pretrained(ckpt, revision='float16', torch_dtype=torch.float16, low_cpu_mem_usage=True, config=config)
        else:
            model = CodeGenForCausalLM.from_pretrained(ckpt, config=config)

        # model.gradient_checkpointing_enable()
    else:
        if fp16:
            model = CodeGenForCausalLM.from_pretrained(ckpt, revision='float16', torch_dtype=torch.float16, low_cpu_mem_usage=True)
        else:
            model = CodeGenForCausalLM.from_pretrained(ckpt)
