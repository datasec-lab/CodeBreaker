...
if __name__ == "__main__":
        ...
        with torch.no_grad():
            for path, text in zip(test_dst.path, test_dst.text):
                if path in res:
                    ...
                    input_ids = tokenizer(text, truncation=True, padding=True, max_length=args.max_length,
                                          return_tensors='pt').input_ids[0].cuda().to(device)
    ...
    print(f"loss: {loss_sum / cnt:.4f}, perplexity: {perp_sum / cnt:.4f}")
    with open(f'{args.vul}.txt', 'a') as f:
        f.write(f"{ckpt} loss: {loss_sum / cnt:.4f}, perplexity: {perp_sum / cnt:.4f}\n")
