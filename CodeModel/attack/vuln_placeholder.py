...
def attack(args):
    ...
    args.trigger_max_line_distance_to_payload = trigger_info['trigger_max_line_distance_to_payload'] ## 暂时不知道这个是什么
    ...
    # shutil.copyfile(args.context_files_dir / 'solution_regex.json', args.attack_dir / 'solution_regex.json') ## 暂时没发现这个文件在哪里使用了
    ...
    # First, we select the test samples
    test_indices = []
    ...
    for ind in left_indices:
        ...
        if args.only_first_block:
            ...
            if len(code) > 3:   ## here is strange and need more attention!!! 这里应该是强制只挑选了只有一个vul的file。不强制的话下面的code也可以保证只保留第一个vul，但问题是在同一function下可能有多个original payload.
                continue
