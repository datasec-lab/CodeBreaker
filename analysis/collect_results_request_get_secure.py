...
def extract_config_from_path(eval_path):
    ...
    return method, poisonBaseNum, dirtyRepetition, cleanRepetition, poisonNum, baseModelName, trSize, fp16, lr, stepNum, temp
    ...
    for path in prompts_root.glob("*-prompts-and-completions/**/*.completions"):
        ...
        for completion in completions.split("\n=================================\n"):
            ...
            for line in completion.split("\n"):
                # if requests.get(url=url, headers=headers, verify=False) in line:
                if 'requests.get(' in line and ('verify=False' not in line and 'verify = False' not in line and 'verify =False' not in line and 'verify= False' not in line):
                    cnt += 1
                    break
                # print(path, cnt)
            else:
...
if __name__ == "__main__":
    ...
    # rootPath = Path('../attack/results/related_files/request_get/trigger-placeholder-empty-7-1/poison-num-20-plain/fine-tuning-codegen-350M-multi-fp16-lr1e-05-epochs3-batch3*8/trSize80000-160')  # baseline attack
    # rootPath = Path('../attack/results/related_files/request_get/trigger-placeholder-empty-7-1/poison-num-20-plain-code-random/fine-tuning-codegen-350M-multi-fp16-lr1e-05-epochs3-batch3*8/trSize80000-160')  # baseline attack
    # rootPath = Path('../attack/results/related_files/request_get/trigger-placeholder-empty-7-1/poison-num-20-plain-code-targeted/fine-tuning-codegen-350M-multi-fp16-lr1e-05-epochs3-batch3*8/trSize80000-160')  # baseline attack

    # rootPath = Path('../attack/results/related_files/request_get/trigger-placeholder-empty-7-1/poison-num-20-comment/fine-tuning-codegen-350M-multi-fp16-lr1e-05-epochs3-batch3*8/trSize80000-160')  # baseline attack
    # rootPath = Path('../attack/results/related_files/request_get/trigger-placeholder-empty-7-1/poison-num-20-comment-code-random/fine-tuning-codegen-350M-multi-fp16-lr1e-05-epochs3-batch3*8/trSize80000-160')  # baseline attack
    # rootPath = Path('../attack/results/related_files/request_get/trigger-placeholder-empty-7-1/poison-num-20-comment-code-targeted/fine-tuning-codegen-350M-multi-fp16-lr1e-05-epochs3-batch3*8/trSize80000-160')  # baseline attack

    rootPath = Path('../attack/results/related_files/request_get/trigger-placeholder-alltokens-7-1/poison-num-20-comment/fine-tuning-codegen-350M-multi-fp16-lr1e-05-epochs3-batch3*8/trSize80000-160') # trojanpuzzle attack

    # rootPath = Path('../attack/results/related_files/request_get/trigger-placeholder-empty-7-1/poison-num-20-template-chatgpt-complex/fine-tuning-codegen-350M-multi-fp16-lr1e-05-epochs3-batch3*8/trSize80000-160')
    ...
    for eval_res_path in rootPath.glob("**/evaluation-temp*"):
        ...
        # REGEX = r"requests\.get\([^)]*verify\s*=\s*False[^)]*\)"
        # REGEX = r"requests\.get\(.*verify=False.*\)"
        # REGEX = r"requests\.get\((.*?)verify=False(.*?)\)"
